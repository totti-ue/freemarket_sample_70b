  ...
  describe "#create" do
    ...
    context 'can save' do
      it "is valid with title, text, category_id, brand_id, status, delivery_charge_id, delivery_origin_id, delivery_size, delivery_method_id, delivery_days, price" do
        item = build(:item, seller_id: user.id, images: [image]) 
        expect(item).to be_valid
      end

      # priceが文字を含むとき
      it "is valid with Numbers with letters" do
        item = build(:item, price: "1000円", seller_id: user.id, images: [image])
        expect(item).to be_valid
      end
    end
    ...
    context 'cannot save' do
      ...
      it "is invalid witout image" do
        item = build(:item, images: [], seller_id: user.id)
        item.valid?
      ...
      it "is invalid without title" do
        item = build(:item, title: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without text" do
        item = build(:item, text: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without status" do
        item = build(:item, status: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without delivery_charge_id" do
        item = build(:item, delivery_charge_id: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without delivery_origin_id" do
        item = build(:item, delivery_origin_id: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without delivery_size" do
        item = build(:item, delivery_size: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without delivery_method_id" do
        item = build(:item, delivery_method_id: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without delivery_days" do
        item = build(:item, delivery_days: nil, seller_id: user.id, images: [image])
        item.valid?
      ...
      it "is invalid without price" do
        item = build(:item, price: nil, seller_id: user.id, images: [image])
        item.valid?
